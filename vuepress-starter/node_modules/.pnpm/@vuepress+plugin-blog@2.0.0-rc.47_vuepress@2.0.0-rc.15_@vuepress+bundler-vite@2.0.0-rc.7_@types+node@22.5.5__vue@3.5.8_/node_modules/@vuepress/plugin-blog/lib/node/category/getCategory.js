 ...
 */
export const getCategory = (pagesMap, store, categoryOptions, slugify, isDebug) => {
    const result = categoryOptions.map(({ key, getter, sorter = () => -1, path = '/:key/', layout = 'Layout', frontmatter = () => ({}), itemPath: itemPathOptions = '/:key/:name/', itemLayout = 'Layout', itemFrontmatter = () => ({}), }) => {
        if (isDebug)
            logger.info(`Generating ${colors.cyan(key)} category.\n`);
        const getItemPath = isFunction(itemPathOptions)
            ? itemPathOptions
            : isString(itemPathOptions)
                ? (name) => itemPathOptions
        ...
        for (const localePath in pagesMap) {
            ...
            for (const category in pageMapStore)
                map[category].indexes = store.addItems(pageMapStore[category]
                    .sort(sorter)
            ...
            if (isDebug) {
                ...
                for (const category in map) {
                    ...
                    infoMessage += `${category}: found ${items.length} items${categoryPath ? ` in path: ${categoryPath}` : ''}\n`;
                }
